...
class MetricHandler(handler.Handler):
    ...
    def get(self, metric_name, host_name, service_description=None):
        ...
        #print("EFFSDDFD" + str(next(response.items()[0][1])))

        metric = live_metric.LiveMetric(
            **self._metric_dict_from_influx_item(response.items()[0], metric_name)
        )
        print (metric)
        print(type(metric))
        return metric

    def get_all(self, metric_name, time, host_name=None, service_description=None):
        """Return all metrics."""
