...
from surveil.api.handlers.status.metrics.influxdb_time_query import build_influxdb_query


class AlertHandler(handler.Handler):
    """Handles requests to alert resources."""

    def get_all(self, host_name=None, service_description=None, time_delta=None):
        influxClient = self.request.influxdb_client
        query = build_influxdb_query("", "ALERT", "*")
        response = influxClient.query(query)
