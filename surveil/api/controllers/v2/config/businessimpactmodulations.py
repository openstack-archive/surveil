...
from surveil.api.handlers.config import buisnessimpactmodulation_handler as bh
...
class BusinessImpactModulationsController(rest.RestController):
    ...
    @wsme_pecan.wsexpose([mod.BuisnessImpactModulation])
    def get_all(self):
    ...
    @util.policy_enforce(['authenticated'])
    @wsme_pecan.wsexpose(mod.BuisnessImpactModulation, unicode)
    ...
    @util.policy_enforce(['authenticated'])
    @wsme_pecan.wsexpose(body=mod.BuisnessImpactModulation, status_code=201)
    def post(self, data):
    ...
    @util.policy_enforce(['authenticated'])
    @wsme_pecan.wsexpose(mod.BuisnessImpactModulation, unicode, status_code=204)
    ...
    @wsme_pecan.wsexpose(mod.BuisnessImpactModulation,
                         unicode,
                         body=mod.BuisnessImpactModulation,
                         status_code=204)
