...
class TestSeparatedIntegrationSurveil(
    ...
    def test_create_host(self):
        ...
        self.get_client().config.hosts.create(
            host_name='integrationhosttest',
            address='127.0.0.1',
        )

        self.get_client().config.reload_config()

        def function():
            status_hosts = self.get_client().status.hosts.list()
            self.assertTrue(
    ...
    def test_delete_host(self):
        ...
        self.get_client().config.hosts.delete(
            'integrationhosttest'
        )
