...
def main():
    ...
    for cfg_file in cfg_files:
        ...
        # Load the config
        file_config = load_config(config_string)
    ...
    surveil_config = transform_config(nagios_config)

    print(json.dumps(surveil_config, indent=4, sort_keys=True))


def load_config(config_string):
    """Given a nagios configuration file, returns surveil configuration"""
    config = {}
...
def transform_config(nagios_config):
    transformed_config = {}
    ...
    for object_type, objects in nagios_config.items():
        for config_object in objects:
            ...
            name_transformed_obj = transform_property_names(
                config_object,
                object_type
            )

            # PROPERTY TYPES TRANSORMATIONS
            type_transformed_obj = transform_property_types(
                config_object,
                object_type
            )
...
def transform_property_types(config_object, object_type):
    transformed_object = {}
...
def transform_property_names(config_object, object_type):
    transformed_object = {}
