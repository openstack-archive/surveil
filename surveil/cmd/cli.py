...
import surveilclient.client
import surveilclient.v1_0.client
...
The environment variable to use are the same as the CLI arguments,
but in upper case, without the leading '--' and with others '-' replaced by '_'.
Example:
...
add('-A', '--api-url', help="The Surveil API URL (http(s)://HOST(:PORT)/(URI)).")
...
def main(args=None):
    ...
    os_hostname = urlparse(cfg.os_auth_url).netloc
    ...
    if False:
        ...
        # TODO:
        for what in (
            'services', 'flavors', 'hypervisors', 'images', 'agents',
            'hosts', 'availability_zones', 'aggregates', 'certs',
        ):
            try:
                print("{} : {}".format(what, getattr(nova_cli, what).list()))
            except Exception as err:
                print("{} failed: {}".format(what, repr(err)))

        services = nova_cli.services.list()


    cli_surveil = surveilclient.client.Client(cfg.api_url)
    #assert isinstance(cli_surveil, surveilclient.v1_0.client.Client)
    #cli_surveil.hosts.list()
    ...
    for ep in endpoints.get('identity', []):
        ...
        cli_surveil.hosts.create(
            ...
            use='linux-keystone',
            address=urlparse(ep['publicURL']).hostname,
