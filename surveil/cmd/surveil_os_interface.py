...
def main():
    ...
    daemon_config = {
        "SURVEIL_API_URL": os.environ.get(
            'SURVEIL_API_URL',
            config.get("surveilosinterface", "SURVEIL_API_URL")
        ),
        "SURVEIL_AUTH_URL": os.environ.get(
            'SURVEIL_AUTH_URL',
            config.get("surveilosinterface", "SURVEIL_AUTH_URL")
        ),
        "SURVEIL_VERSION": os.environ.get(
            'SURVEIL_VERSION',
            config.get("surveilosinterface", "SURVEIL_VERSION")
        ),
        "RABBIT_HOST": os.environ.get(
            'RABBIT_HOST',
            config.get("surveilosinterface", "RABBIT_HOST")
        ),
        "RABBIT_PORT": int(os.environ.get(
            'RABBIT_PORT',
            config.get("surveilosinterface", "RABBIT_PORT")
        )
        ),
        "QUEUE": os.environ.get(
            'QUEUE',
            config.get("surveilosinterface", "QUEUE")
        ),
        "RABBIT_USER": os.environ.get(
            'RABBIT_USER',
            config.get("surveilosinterface", "RABBIT_USER")
        ),
        "RABBIT_PASSWORD": os.environ.get(
            'RABBIT_PASSWORD',
            config.get("surveilosinterface", "RABBIT_PASSWORD")
        ),
        "SURVEIL_OS_AUTH_URL": os.environ.get(
            'SURVEIL_OS_AUTH_URL',
            config.get("surveilosinterface", "SURVEIL_OS_AUTH_URL")
        ),
        "SURVEIL_OS_USERNAME": os.environ.get(
            'SURVEIL_OS_USERNAME',
            config.get("surveilosinterface", "SURVEIL_OS_USERNAME")
        ),
        "SURVEIL_OS_PASSWORD": os.environ.get(
            'SURVEIL_OS_PASSWORD',
            config.get("surveilosinterface", "SURVEIL_OS_PASSWORD")
        ),
        "SURVEIL_OS_TENANT_NAME": os.environ.get(
            'SURVEIL_OS_TENANT_NAME',
            config.get("surveilosinterface", "SURVEIL_OS_TENANT_NAME")
        ),
        "SURVEIL_DEFAULT_TAGS": os.environ.get(
            'SURVEIL_DEFAULT_TAGS',
            config.get("surveilosinterface", "SURVEIL_DEFAULT_TAGS")
        ),
        "SURVEIL_NETWORK_LABEL": os.environ.get(
            'SURVEIL_NETWORK_LABEL',
            config.get("surveilosinterface", "SURVEIL_NETWORK_LABEL")
        ),
    }
